...
def _drawOuterCadran(alpha0, alpha1, centerx, centery, r, width):
    if alpha0 == alpha1:
...
class SpeedMeter(Widget):
    ...
    def _drawLabels(self, alpha0, alpha1, centerx, centery, r):
        # For some reason Line(circle=) is rotated 90 degrees ???
        alpha0 += 90
        alpha1 += 90

        labelStr = self.labelStr
        labels = [Label(labelStr(i), bold=True)
                      for i in xrange(self.min, self.max + 1, self.tick)]
        for _ in labels: _.refresh()

        deltaAlpha = radians((alpha1 - alpha0) / float(len(labels) - 1))
        ...
        if subtick:
            ...
            subDeltaAlpha = None
        for label in labels:
            first = label is labels[0]
            last = label is labels[-1]
            c = cos(alpha)
            s = sin(alpha)
    ...
    def _update(self, *args):
        ...
        with self.canvas:
            ...
            _drawOuterCadran(alpha0, alpha1, centerx, centery, r, width)

            if alpha0 == alpha1:
                alpha1 = alpha1_ = alpha0 + 360
            self._drawLabels(alpha0, alpha1, centerx, centery, r)
            #
